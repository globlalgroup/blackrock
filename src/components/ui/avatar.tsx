import cn from '@/utils/cn';
import Image from '@/components/ui/image';
import { StaticImageData } from 'next/image';

interface AvatarProps {
  image: StaticImageData | string;
  alt: string;
  className?: string;
  size?: SizeNames;
  shape?: ShapeNames;
  width?: number;
  height?: number;
}

type ShapeNames = 'rounded' | 'circle';
type SizeNames = 'xl' | 'lg' | 'md' | 'sm' | 'xs';

const sizes: Record<SizeNames, string[]> = {
  xl: [
    'border-white border-[5px] h-24 w-24 sm:h-28 sm:w-28 md:h-32 md:w-32 3xl:h-40 3xl:w-40 3xl:border-8 shadow-large',
  ],
  lg: ['border-white border-4 h-20 w-20 lg:h-24 lg:w-24'],
  md: ['border-white h-10 w-10 drop-shadow-main border-3'],
  sm: ['border-white h-8 w-8 border-2 shadow-card'],
  xs: ['h-6 w-6'],
};

const shapes: Record<ShapeNames, string[]> = {
  rounded: ['h-16 w-16 rounded-lg bg-white/20 p-2 backdrop-blur-[40px]'],
  circle: ['rounded-full'],
};

const defaultFallback = '/default-avatar.png';

const defaultDimensions: Record<SizeNames, number> = {
  xl: 128,
  lg: 96,
  md: 48,
  sm: 32,
  xs: 24,
};

export default function Avatar({
  image,
  alt,
  className,
  size = 'md',
  shape = 'circle',
  width,
  height,
}: AvatarProps) {
  const sizeClassNames = sizes[size];

  // ✔ Corrige la construcción del src
  const src =
    typeof image === 'string' && !image.startsWith('http') && !image.startsWith('/')
      ? `/profile-images/${image}`
      : image;

  const w = width ?? defaultDimensions[size];
  const h = height ?? defaultDimensions[size];

  return (
    <figure
      className={cn(
        'relative shrink-0 overflow-hidden',
        className,
        shapes[shape],
        shape === 'circle' && sizeClassNames
      )}
    >
      <Image
        src={src}
        alt={alt}
        width={w}
        height={h}
        priority
        placeholder={typeof image === 'string' ? undefined : 'blur'}
        className={shape === 'circle' ? 'rounded-full' : 'rounded-[6px]'}
        onError={(e) => {
          (e.target as HTMLImageElement).src = defaultFallback;
        }}
      />
    </figure>
  );
}

